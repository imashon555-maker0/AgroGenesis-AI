  ...
  return (
    <div className="space-y-5 p-4 lg:p-6 animate-fade-in-up">
      <div className={"flex items-center justify-between " + (isPhone ? "flex-col items-start gap-3" : "")}>
        <div>
          <h2 className="text-lg font-bold text-earth-100">Управление полями</h2>
          <p className="text-field-300 text-xs mt-1">Управление границами полей и зонами</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="bg-canopy-900/60 border border-canopy-700/40 rounded-xl p-8 text-center animate-fade-in">
          <p className="text-field-300 text-sm">Пока нет полей. Click "+ Добавить поле" to create your first field.</p>
        </div>
      ) : (
        <>
          {/* Summary Stats */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            <div className="bg-canopy-900/40 border border-canopy-700/40 rounded-xl p-4">
              <p className="text-[10px] text-field-300 uppercase mb-1">Всего полей</p>
              <p className="text-xl font-bold text-earth-100">{fields.length}</p>
            </div>
            <div className="bg-canopy-900/40 border border-canopy-700/40 rounded-xl p-4">
              <p className="text-[10px] text-field-300 uppercase mb-1">Общая площадь</p>
              <p className="text-xl font-bold text-earth-100">{fields.reduce((s, f) => s + (f.area_ha || 0), 0).toFixed(0)} ha</p>
            </div>
            <div className="bg-canopy-900/40 border border-canopy-700/40 rounded-xl p-4">
              <p className="text-[10px] text-field-300 uppercase mb-1">Ср. НДВИ</p>
            ...
            <div className="bg-canopy-900/40 border border-canopy-700/40 rounded-xl p-4">
              <p className="text-[10px] text-field-300 uppercase mb-1">Всего зон</p>
