...
const NAV_ITEMS = [
  ...
  { path: "/telemetry", label: "Телеметрия", icon: Activity },
  { path: "/imagery", label: "Съёмка", icon: Satellite },
  { path: "/prescriptions", label: "Рецептуры", icon: Bot },
  { path: "/ecofin", label: "EcoFin", icon: Leaf },
  { path: "/settings", label: "Настройки", icon: Map },
];
    ...
    <div className="flex h-screen overflow-hidden bg-field-950">
      {isDesktop && (
        <aside onMouseEnter={() => setExpanded(true)} onMouseLeave={() => setExpanded(false)} className={"flex flex-col z-50 border-r transition-all duration-200 ease-in-out bg-field-900/80 border-canopy-800/60 " + (expanded ? "w-[220px]" : "w-[64px]")}>
          ...
          {expanded && <div className="px-4 py-3 border-t border-canopy-800/60"><p className="text-[10px] text-field-300">v0.1.0 Трек EcoFin</p></div>}
        </aside>
      )}
      {isTablet && (
