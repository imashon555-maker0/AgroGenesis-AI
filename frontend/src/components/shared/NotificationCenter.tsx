...
function genAlerts(fields: any[]): Alert[] {
  ...
  for (const f of fields) {
    for (const z of f.zones || []) {
      ...
      if (n < NDVI_CRIT) a.push({ id: f.id+"-"+z.id+"-c", severity: "critical", message: "NDVI "+n.toFixed(2)+" - critical stress", fieldName: f.name, zoneLabel: z.zone_label });
      else if (n < NDVI_WARN) a.push({ id: f.id+"-"+z.id+"-w", severity: "warning", message: "NDVI "+n.toFixed(2)+" - below optimal", fieldName: f.name, zoneLabel: z.zone_label });
    }
    const ns = (f.zones||[]).map((z:any)=>z.mean_ndvi||0.5);
    const sp = Math.max(...ns)-Math.min(...ns);
    if (sp>0.3 && f.zones.length>=2) a.push({ id:f.id+"-v", severity:"warning", message:"NDVI spread "+sp.toFixed(2)+" across zones", fieldName:f.name, zoneLabel:"All" });
  }
    ...
    <div className="relative">
      ...
      {open && <>
        ...
        <div className="absolute right-0 top-full mt-2 w-[320px] max-h-[400px] overflow-auto bg-[#161e18] border-2 border-canopy-600 shadow-[0_16px_48px_0px_rgba(0,0,0,0.9)] rounded-xl shadow-2xl z-50 animate-pop-in">
          <div className="sticky top-0 bg-[#161e18] border-b border-canopy-700/50 border-canopy-800/60 px-4 py-3 flex items-center justify-between"><h3 className="text-sm font-bold text-earth-50">Alerts</h3><span className="text-[10px] text-field-300">{alerts.length} active</span></div>
          {alerts.length === 0 ? <div className="p-6 text-center"><Leaf size={24} className="mx-auto text-agro-500 mb-2" /><p className="text-xs text-field-300">All fields healthy</p></div> : alerts.map(alert => (
            <div key={alert.id} className="px-4 py-3 border-b border-canopy-800/60 last:border-0">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0"><p className="text-xs text-earth-50">{alert.fieldName} - Zone {alert.zoneLabel}</p><p className="text-[11px] text-field-200 mt-0.5 truncate">{alert.message}</p></div>
